...
import type { PriceAdjustment } from "~/types/listing";
...
<template>
  <div>
    <h3 class="mb-4 text-xl font-bold">Seasonal Price Adjustments</h3>

    <div class="space-y-4">
      <UCard v-for="(adjustment, index) in priceAdjustments" :key="index">
        <div
          class="grid grid-cols-1 gap-4 md:grid-cols-[1fr_180px_auto] md:items-end"
        >
            ...
            <UPopover :content="{ align: 'start' }">
              ...
              <template #content>
                <UCalendar
                  ...
                  range
                  @update:model-value="
                    setDateRange(index, $event as DateRangeValue)
                  "
                />
              </template>
            </UPopover>
