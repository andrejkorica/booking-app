...
const selectedUnits = computed(() =>
  listingUnits.value.filter(
    (unit: ListingUnit) => unit.quantity > 0 && unit.pricePerNight > 0,
  ),
);
...
const lowestPrice = computed(() => {
  ...
  return Math.min(
    ...selectedUnits.value.map((unit: ListingUnit) => unit.pricePerNight),
  );
});
...
<template>
  <div>
    <h3 class="mb-4 text-xl font-bold">Units and Prices</h3>
    ...
    <div class="mb-8 rounded-xl bg-slate-50 p-4 text-sm text-slate-600">
      ...
      <span v-else> Add at least one unit with quantity and price. </span>
    </div>
  </div>
</template>
