...
const priceRange = ref<{
  ...
  min:
    typeof route.query.minPrice === "string"
      ? Number(route.query.minPrice)
      : null,
  max:
    typeof route.query.maxPrice === "string"
  ...
  <aside class="mb-5 rounded-xl border border-slate-200 bg-white">
    ...
    <div class="space-y-5 p-4">
      <div class="border-slate-200">
        <FilteringBarAmenities v-model="selectedAmenities" />
      </div>

      <div class="border-t border-slate-200 pt-4">
        <FilteringBarPriceRange v-model="priceRange" />
      </div>

      <div class="border-t border-slate-200 pt-4">
        <FilteringBarRating v-model="rating" />
      </div>
    </div>
  </aside>
</template>
